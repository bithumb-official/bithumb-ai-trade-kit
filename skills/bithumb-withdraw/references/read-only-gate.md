...
Pre-flight gate for withdrawal write operations. Run this **before** any other withdrawal write pre-flight step once a write intent is confirmed — before checking assets, wallet status, withdrawal chance, allowed addresses, in/out fees, receiver details, or a pending withdrawal to cancel.

Rationale: `read_only` is a purely local value — the effective profile's `read_only` in `~/.bithumb/config.toml`, with no env var or live API involved. Checking it up front avoids wasting withdrawal pre-flight work that would be thrown away when the write is ultimately blocked.

> **This gate is your responsibility — there is no code block behind it.** The runner does **not** intercept write-preflight reads; in read-only mode the withdrawal lookups still run and hit the network. What helps you is `config show --json`: when the effective profile is read-only it returns a `read_only_notice` (`blocked_for_writes`, `write_capable_profiles`, `guidance`). Read it up front and skip the withdrawal pre-flight lookups yourself — running them wastes calls on a write that cannot execute. See CONTEXT.md *write-preflight read* and ADR 0014.

## Steps

1. **Confirm the request is a write operation.** Reads (`withdraw chance`, `withdraw addresses`, `withdraw get`, `withdraw list`, `withdraw list-krw`) need no gate — skip straight to them. The withdrawal write operations blocked at execution in read-only mode are:
   - `withdraw coin`
   - `withdraw krw`
   - `withdraw cancel`
...
3. **Read the effective profile's `read_only` (local, no network):**
   ...
   (If the user named a profile for this withdrawal, check that one: `bithumb config show --profile <name> --json` and read `settings.read_only`. If `settings` is **null** the profile does not exist — do not proceed and do not treat the missing value as "writes allowed". Run `bithumb config show --json` to list the configured profiles, show them to the user, and ask for a valid name.)

4. **When `read_only` is true — stop, do not run withdrawal lookups.** The write would be blocked at execution anyway. Instead:
   - Tell the user the effective profile is read-only, so real withdrawals or withdrawal cancellations are not possible with it.
