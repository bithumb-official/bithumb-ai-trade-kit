...
> **Identifier field**: in the raw MCP response (and CLI `--json`), the order identifier arrives under the `uuid` key. Present it to the user as `order_id`.
...
When `order_not_found` is returned, **always use `trade get --order-id <id>` to look up the order directly** instead of querying via `trade list`. `trade list --limit N` may miss the order due to exceeding the limit or a missing market filter.
...
Order-placement examples are in the `trade place` section above. Common follow-up commands:
